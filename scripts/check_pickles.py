...
parser = argparse.ArgumentParser(description='Check critical ETL pickles for presence/emptiness')
parser.add_argument('--no-abort', action='store_true', help='Do not abort on missing/empty pickles; only print warnings')
parser.add_argument('--no-emoji', action='store_true', help='Do not print emoji characters (safe for non-utf8 consoles)')
    ...
    if not ppath.exists():
        ...
        try:
            ...
            if df is None or df.empty:
                print(f"[WARN] Empty pickle: {ppath} (shape: {None if df is None else df.shape})")
                missing_or_empty.append(str(ppath))
            else:
                if emoji:
                ...
                if expected_cols:
                    ...
                    if missing_cols:
                        print(f"[WARN] Pickle {ppath} lacks expected columns: {missing_cols}")
                        missing_or_empty.append(str(ppath))
