...
try:
    ...
    for anio in anios_epf:
        ...
        print(f"   {anio}: {len(registros_anio)} registros | "
              f"{quintiles} quintiles | {grupos_gasto} grupos de gasto")
        
        # Validar que todos los quintiles están presentes
        quintiles_esperados = {'Quintil 1', 'Quintil 2', 'Quintil 3', 'Quintil 4', 'Quintil 5', 'Total'}
        quintiles_reales = set(registros_anio['Quintil'].unique())
        
        if not quintiles_reales >= {'Quintil 1', 'Quintil 2', 'Quintil 3', 'Quintil 4', 'Quintil 5'}:
    ...
    if anios_faltantes:
        print(f"   ⚠️  Años sin datos EPF (esperado en diseño bienal): {sorted(anios_faltantes)}")
    ...
    print(f"   ⚠️  Para análisis temporal continuo, cruzar con IPC anual (disponible todos los años)")
...
try:
    ...
    print(f"📅 Rango temporal: {df_ipc_anual['Anio'].min()} - {df_ipc_anual['Anio'].max()}")
    ...
    if 'IPC_Medio_Anual' in df_ipc_anual.columns:
        ...
        if not ipc_2021.empty:
            ...
            if 99.5 <= valor_2021 <= 100.5:
                ...
                print(f"   ⚠️  Base 2021 dudosa: IPC_2021 = {valor_2021:.2f} (esperado ≈100)")
    ...
    if 'Inflacion_Anual_%' in df_ipc_anual.columns:
        ...
        df_ipc_anual_sorted['Inflacion_Calculada'] = df_ipc_anual_sorted['IPC_Medio_Anual'].pct_change() * 100
        
        # Comparar (solo años con ambos valores)
        comparacion = df_ipc_anual_sorted[['Anio', 'Inflacion_Anual_%', 'Inflacion_Calculada']].dropna()
        comparacion['Diferencia'] = abs(comparacion['Inflacion_Anual_%'] - comparacion['Inflacion_Calculada'])
        
        max_diferencia = comparacion['Diferencia'].max()
        
        if max_diferencia < 0.1:
            print(f"   ✅ Inflación calculada correctamente (max diferencia: {max_diferencia:.4f}pp)")
        else:
            print(f"   ⚠️  Discrepancias detectadas (max diferencia: {max_diferencia:.2f}pp)")
            print("\n   Años con mayor discrepancia:")
            print(comparacion.nlargest(5, 'Diferencia')[['Anio', 'Inflacion_Anual_%', 'Inflacion_Calculada', 'Diferencia']].to_string(index=False))
...
try:
    ...
    if 'Tipo_Metrica' in df_ipc_sect.columns:
        ...
        var_anual = df_ipc_sect[df_ipc_sect['Tipo_Metrica'].str.contains('ariación anual', case=False, na=False)]
        ...
        if not var_anual.empty:
            ...
            if nulls_var_anual == 0:
                ...
                print(f"   ✅ Esta métrica es segura para análisis de inflación diferencial")
            else:
...
try:
    ...
    print(f"   ECV (Gini):  {min(anios_gini)} - {max(anios_gini)} ({len(anios_gini)} años)")
    print(f"   IPC anual:   {min(anios_ipc)} - {max(anios_ipc)} ({len(anios_ipc)} años)")
    print(f"   EPF bienal:  {min(anios_epf_set)} - {max(anios_epf_set)} ({len(anios_epf_set)} años)")
    ...
    if len(todos) >= 5:
        ...
        print(f"   ✅ Suficientes puntos temporales para tendencias ({len(todos)} años)")
    else:
        print(f"   ⚠️  Solo {len(todos)} años con las 3 fuentes → análisis limitado")
    
    if len(gini_ipc) >= 10:
        print(f"   ✅ Análisis ECV+IPC: usar años {min(gini_ipc)}-{max(gini_ipc)} ({len(gini_ipc)} años)")
    else:
