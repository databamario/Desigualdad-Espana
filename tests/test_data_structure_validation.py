...
def test_execute_03_analisis_inflacion_diferencial_basic(tmp_path: Path):
    ...
    assert (
        "df_renta" in ns or "df_pivot_deciles" in ns
    ...
    if "df_pivot_deciles" not in ns and "df_renta" in ns:
        ...
        val_col = (
            ...
            else next((c for c in dr.columns if "Renta" in str(c)), None)
        )
        ...
        ns["df_pivot_deciles"] = normalize_decile_columns(
            ns.get("df_pivot_deciles", pd.DataFrame())
        )
    ...
    ), f"Missing decile columns: {list(pivot.columns)}"
    ...
    assert (
        2019 in years and 2023 in years
    ), f"Years missing in df_pivot_deciles index: {years}"
