...
def test_normalize_umbral_pobreza_euros_alias():
    df = pd.DataFrame({'Anio': [2020], 'Tipo_Hogar': ['Hogares de una persona'], 'Umbral_Pobreza_Euros': [1000]})
...
def test_normalize_umbral_real_base_alias():
    df = pd.DataFrame({'Anio': [2020], 'Tipo_Hogar': ['Hogares de una persona'], 'Umbral_Real_€_Base': [1200]})
    df2 = normalize_umbral_dataframe(df)
    assert 'Umbral_Euros' in df2.columns
...
def test_normalize_decile_columns_int_and_str():
    df = pd.DataFrame({1: [100, 200], '2': [150, 250], 'Decil 3': [120, 220]}, index=[2019, 2020])
