...
class TestDeflactacion:
    ...
    def test_deflactor_aumenta_con_inflacion_positiva(self):
        ...
        assert deflactor[2009] > deflactor[2008], "El deflactor debe aumentar con inflación positiva"
        assert abs(deflactor[2009] - 1.03) < 0.001, "Deflactor 2009 debe ser aproximadamente 1.03"
    ...
    def test_renta_real_menor_con_inflacion(self):
        ...
        assert renta_real < renta_nominal, "Renta real debe ser menor que nominal con inflación"
        assert abs(renta_real - 8333.33) < 1, "Renta real debe ser aproximadamente 8333.33"
...
class TestValidacionDatos:
    ...
    def test_validar_rango_detecta_valores_fuera_rango(self):
        ...
        df = pd.DataFrame({
            'Gini': [33.2, 150.0, 34.1]  # 150 está fuera de rango
        })
...
class TestConsistenciaResultados:
    ...
    def test_gini_y_s80s20_correlacionan_positivamente(self):
        ...
        df = pd.DataFrame({
            'Gini': [32.0, 33.0, 34.0, 35.0],
            'S80S20': [5.0, 5.3, 5.6, 5.9]
        })
        
        correlacion = df['Gini'].corr(df['S80S20'])
        assert correlacion > 0.7, f"Correlación Gini-S80S20 debe ser > 0.7, obtenida: {correlacion}"
    ...
    def test_umbral_real_consistente_con_inflacion(self):
        ...
        assert umbral_real_2023 < umbral_nominal_2023, "Umbral real debe ser menor que nominal"
