...
import pytest
import pandas as pd
from VALIDACION_AUTOMATICA_TEMPLATE import validar_rangos, comparar_fuentes, ValidationWarnings
...
def test_validar_rangos_outliers():
    ...
    assert any(w['mensaje'].startswith('Valores fuera de rango') for w in warnings.warnings)
...
def test_comparar_fuentes_discrepancia():
    ...
    assert (dif > 0.05).any()
    assert any('Discrepancias significativas' in w['mensaje'] for w in warnings.warnings)
