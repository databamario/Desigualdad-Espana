...
def validar_rangos(df, columna, min_val, max_val, warnings):
    ...
    if not outliers.empty:
        warnings.add(
            f"Valores fuera de rango en {columna}", contexto=outliers.index.tolist()
        )
    return outliers
...
def comparar_fuentes(df1, df2, columna, warnings, tolerancia=0.05):
    ...
    if (dif > tolerancia).any():
        warnings.add(
            f"Discrepancias significativas en {columna} entre fuentes",
            contexto=dif[dif > tolerancia].index.tolist(),
        )
    return dif
...
def validar_coherencia_temporal(df, columna, warnings, max_delta=0.2):
    ...
    if (delta > max_delta).any():
        warnings.add(
            f"Cambios interanuales atípicos en {columna}",
            contexto=delta[delta > max_delta].index.tolist(),
        )
    return delta
